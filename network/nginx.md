...
* usage
  ...
  + обработка статического контента, одинакового для всех пользователей (карточки товаров в онлайн-магазине, новостные сайты)
  + кеширование и стриминг видео
  + построение CDN-сетей
  + хостинг нескольких сайтов на одном сервере
* как устроен
  + асинхронная архитектура, запросы обрабатываются на разных этапах -> скорость (до него запросы каждого пользователя обрабатывались по отдельности, требовалось много процессов)
...
### Config
* `/etc/nginx/conf.d/`
, которая традиционно используется для тех же целей. Файлы в этой директории, как правило, включаются в основной файл конфигурации Nginx через директиву include. Это позволяет разделять конфигурации для различных сайтов или сервисов на более мелкие и управляемые части.
* `/etc/nginx/http.d/` в некоторых конфигурациях nginx для хранения отдельных файлов конфигураций виртуальных хостов или других конфигурационных файлов, связанных с обработкой HTTP-запросов
  + альтернатива или дополнение к `/etc/nginx/conf.d/`
Основное назначение:
Хранение отдельных конфигурационных файлов для различных доменов (виртуальных хостов).
Подключение отдельных конфигураций для обработки HTTP-запросов.
Однако данная структура каталогов может варьироваться в зависимости от дистрибутива Nginx и операционной системы.

Да, для HTTPS-запросов также могут использоваться файлы конфигурации из директории /etc/nginx/http.d/, если в этих файлах прописаны настройки для SSL (HTTPS).
Для настройки HTTPS необходимо добавить в конфигурационный файл (или в файлы, которые хранятся в папке /etc/nginx/http.d/) следующие элементы:
Слушатель для порта 443 (по умолчанию порт для HTTPS):
server {
    listen 443 ssl;
...
SSL-сертификаты и ключи, которые нужны для шифрования соединений.
Таким образом, если конфигурации для HTTPS-запросов расположены в файлах в папке /etc/nginx/http.d/, Nginx будет обрабатывать и HTTPS-запросы на основе этих настроек. Это не обязательно — многие используют другие директории, такие как /etc/nginx/conf.d/ или отдельные файлы конфигурации.
Главное, чтобы Nginx был настроен на использование правильных конфигураций через директивы include.
