...
* взаимодействует с docker-демоном
...
* технология для хранения слоев: Union File System (UnionFS)
  ...
  + Union file system позволяет файлам и директориями из разных файловых систем (разным ветвям) прозрачно накладываться, создавая файловую систему
...
* docker клиент говорит docker демону запустить контейнер с помощью программы docker или с помощью REST API
  ...
  + docker скачивает образ ubuntu (проверяет наличие образа на локальной машине, если нет — то скачивает его с Docker Hub)
  + docker создает контейнер
  + docker инициализирует файловую систему
  + docker монтирует read-only уровень
  + docker инициализирует сеть/мост: создает сетевой интерфейс, который позволяет docker-у общаться хост машиной
  + docker устанавливает IP адрес: находит и задает адрес
  + docker запускает ваше приложение
  + docker обрабатывает и выдает вывод вашего приложения и логирует стандартный вход, вывод и поток ошибок
