...
* ожидает запросов через REST API (**Docker Engine API**) от Клиента
...
* управляет :
  ...
  + процессами коммуникации между контейнерами
* следит за состоянием других компонентов
* собирает всё необходимое для запуска приложения в образ
* использует собственный драйвер `Docker runc`, который связан со следующими механизмами ядра:
...
* консольный или графический клиент
* общается с сервером через REST API или с помощью командной строки
* пользователи взаимодействуют с Docker daemon и отправляют ему команды, создают контейнеры, управляют ими
* API к докер-демону
...
* набор файлов, соединенный с настройками, с помощью которого можно создать экземпляры, которые запускаются в контейнерах
