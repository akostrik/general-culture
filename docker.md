...
* запущенный образ, is started by running an image
  ...
  + образ = CD-диск, с которого будет установлен софт, контейнер = запущенная копия образа
* исполняемый пакет
* содержит все для запуска (системные программы, библиотеки, код, среды исполнения, настройки, необходимые для запуска софта программы и библиотеки)
* runtime-сущность
* one container = one service = одно развёрнутое и запущенное приложение
* специальный слой для записи и временного хранения информации
...
* технологии ядра Linux для изоляции контейнеров:
  + Linux Namespaces контролируют доступ к структурам данных ядра
...
* Данные записываются в специальный слой «сверху» контейнера и при удалении контейнера удаляются, но данные можно сохранить с помощью:
