...
* когда лучше запускать nginx в Dockerfile:
...
* когда лучше запускать nginx в docker-compose:
  ...
  + проект требует гибкого управления, масштабирования и настройки среды на уровне нескольких контейнеров
  + менять конфигурацию без пересборки (например, перемаппить порты или подключить другие сервисы) без пересборки образа
  + легко масштабировать через docker-compose up --scale
  + создание сетей для взаимодействия контейнеров
* volumes
  + Формат записи: `хостовая_директория_или_файл:контейнерная_директория_или_файл`
  + монтирование директорий и файлов между хостовой машиной (где запущен Docker) и контейнером
  + делиться файлами и данными между контейнером и хостом, обеспечивая сохранность данных или упрощая работу с конфигурациями
  + управлять, какие данные и файлы хранятся вне контейнера и каким образом они взаимодействуют с контейнером
  + сохранять данные вне контейнера, что позволяет пережить перезапуски или удаление контейнеров
  + Named Volumes
    - `volumes: mydata:/usr/share/nginx/html`, mydata — именованный том, создается Docker и монтируется в контейнере по /usr/share/nginx/html
    - создаются и управляются Docker
    - содержимое сохраняется вне контейнера на хосте
    - могут использоваться повторно разными контейнерами, например, для хранения базы данных или файлов
    - данные сохраняются, даже если контейнер удален
    - для хранения данных, чтобы они сохранялись между перезапусками контейнеров
    - можно объявить `volumes: mydata:`, чтобы Docker создал их автоматически
  + Bind Mounts
    - `volumes: - ./myapp:/usr/share/nginx/html`
    - директория или файл на хостовой машине напрямую монтируются в контейнер
    - полезно, когда нужно поделиться существующими файлами или конфигурациями
    - позволяют работать с файлами локально и видеть изменения сразу в контейнере
    - полезно, когда нужно иметь доступ к исходным данным или кода на хосте, чтобы изменения сразу отражались в контейнере
  ...
  + Anonymous Volumes
    ...
    - если нужно временно хранить данные
    - их нельзя использовать повторно между контейнерами
    - создать временный том
    - указать контейнерный путь без хостовой директории
    - Docker будет управлять его данными до удаления контейнера
