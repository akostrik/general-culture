...
* `lsof -nP -i | grep LISTEN` тоже писок прослушиваемых портов
* `lsof -nP -i` все доступные соединения
* `ss -ltupn` почти как netstat
* `iptables -L` правила брандмауэра iptables, чтобы понять, какие порты закрыты с его помощью
* `nmap -sT 192.168.1.3` проверить список портов Ubuntu так, как бы это делал злоумышленник при попытке атаки на вашу систему (узнали свой ip с помощью ifconfig)
