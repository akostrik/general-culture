...
* как работает
  ...
  + если обрабатывать запросы каждого пользователя по отдельности, серверу придется выполняет много процессов, сайт работает медленно
  + Nginx обходит это: асинхронная архитектура, запросы обрабатываются на разных этапах => скорость обработки
