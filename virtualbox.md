...
### Portforwarding
* В таблице Port Fprwardign Rules нельзя указывать адрес хоста 127.0.0.1
...
* Как правило нет нужды в пробросе портов
  + достаточно выставить тип подключения в настройках сетевого адаптера
    - тип подключения "Виртуальный адаптер хоста", если к виртуалке нужен доступ только с хоста => по умолчанию виртуалке присваивается адрес из сети 192.168.56.0/24, на хосте при этом 192.168.56.1
