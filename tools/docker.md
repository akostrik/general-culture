...
* не надо `tail -f` для отслеживания логов контейнера, потому что
  ...
  + позволяет просматривать файлы напрямую
  + нагрузка на систему
* не надо запусать процессы в фоновом режиме, потому что  
  ...
  + `docker logs` собирает и отображает логи в реальном времени, что может быть недоступно для фоновых процессов
